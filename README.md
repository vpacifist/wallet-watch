...
По умолчанию кнопка `START` запускает серверную simulation job через `/api/simulations`. Сам расчет выполняется в headless Chromium на сервере, поэтому вкладку браузера можно закрыть, а позже открыть страницу снова и увидеть последний статус.
